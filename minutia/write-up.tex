...
Data bifurcating could be done with 20/80 percents between training set and test set in classic machine learning. Other used names for test set are such as validation set, evaluating set, etc. Model optimization is discussed into two areas, \textit{algorithmic} or \textit{parametric} optimization. In deep learning where in the models heavily depend on \textit{hyperparameters}, test set could be served to "train" hyperparameters prior to optimization. 
